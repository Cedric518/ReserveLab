...
def backtesting(company_code, method_name, factor_average, pattern_source, valuation_year):
    ...
    square, _ = ut.get_squares(company_code, valuation_year)
    ...
    error = calculate_error(estimates, square)
    error_industry = calculate_error(estimates_industry, square)
...
def calculate_error(estimates, square):
    error = pd.DataFrame()
    error['error'] = estimates['estimated_cumulative_paid'] - square['10']
    error['error_percentage'] = error['error'] / square['10'] * 100
    print('\nERROR CALCULATION')
    print(error)
    return error